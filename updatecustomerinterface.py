        ...
        if i=="":
            ...
            if len(mb)==10:
                if m=="Temporary":
                    ...
                    d="20%"
                cur.execute("update custinfo set cust_name='"+str(n)+"' where cust_id="+str(i))
                cur.execute("update custinfo set address='"+str(a)+"' where cust_id="+str(i))
                cur.execute("update custinfo set mobile_no='"+str(mb)+"' where cust_id="+str(i))
                cur.execute("update custinfo set membership='"+str(m)+"' where cust_id="+str(i))
                cur.execute("update custinfo set discount='"+str(d)+"' where cust_id="+str(i))
                cn.commit()
...
def findrec():
    try:
        ...
        messagebox.showinfo("RMS","Please Enter Customer ID to Find")
wind=Tk()
...
discount=StringVar()
################################
l1=tkinter.Label(wind,text="Update Customer",fg="red",bg=None,font=("Segoe Print",14))
l1.pack()
cur.execute("select * from custinfo")
result=cur.fetchall()
for i in result:
...
idd=ttk.Entry(wind,state="noraml",textvariable=identity).place(x=150,y=75)
